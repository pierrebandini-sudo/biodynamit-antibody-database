/* BioDynaMit v11.7.5.4 — stockage unifié, ajout par URL fournisseur, recherche hybride par référence et gestion des références.
   ...
   - Ajout par URL fournisseur, avec repli référence catalogue + fournisseur, fiche adaptative et validation humaine.
...
(function(){
  ...
  /* --------------------- Product creation by supplier URL --------------------
     v11.7.5.4 scope rule: this Reader/enrichment UI exists ONLY inside an explicit
  ...
  const SUPPLIER_ALIASES={
    'thermo fisher':'thermo fisher scientific','thermofisher':'thermo fisher scientific','thermo fisher scientific':'thermo fisher scientific','invitrogen':'thermo fisher scientific',
    ...
    'sigma aldrich':'merck sigma aldrich','sigma-aldrich':'merck sigma aldrich','millipore':'merck sigma aldrich','milliporesigma':'merck sigma aldrich','merck sigma aldrich':'merck sigma aldrich','merck / sigma-aldrich':'merck sigma aldrich',
  ...
  function normalizeProductUrl(value){const raw=String(value||'').trim();if(!raw)return'';return /^https?:\/\//i.test(raw)?raw:`https://${raw.replace(/^\/\//,'')}`}
  ...
  function sameTypeRefs(type,ref,supplier=''){
    const cr=compact(ref);if(!cr)return[];
    const rows0=type===PRIMARY?tr('Antibodies'):tr('InventoryItems').filter(i=>i.InventoryType===type);
    const hits=rows0.filter(r=>compact(r.CatalogNumber)===cr);
    if(!supplier)return hits;
    return hits.filter(r=>supplierSame(r.Supplier,supplier));
  }

  /* Recherche locale transversale : une référence peut déjà exister dans
     les anticorps primaires, secondaires ou un futur inventaire générique. */
  function localReferenceMatches(ref,supplier=''){
    const cr=compact(ref);if(!cr)return[];
    const out=[];
    tr('Antibodies').forEach(row=>{
      if(compact(row.CatalogNumber)!==cr)return;
      out.push({kind:'primary',inventoryType:PRIMARY,row,supplierMatch:!supplier||supplierSame(row.Supplier,supplier)});
    });
    tr('InventoryItems').forEach(row=>{
      if(compact(row.CatalogNumber)!==cr)return;
      out.push({kind:'inventory',inventoryType:row.InventoryType||'',row,supplierMatch:!supplier||supplierSame(row.Supplier,supplier)});
    });
    return out.sort((a,b)=>{
      if(a.supplierMatch!==b.supplierMatch)return a.supplierMatch?-1:1;
      if(activeRaw(a.row)!==activeRaw(b.row))return activeRaw(a.row)?-1:1;
      return String(a.row.Name||a.row.Code||'').localeCompare(String(b.row.Name||b.row.Code||''),'fr');
    });
  }
  function localTypeLabel(hit){
    if(hit?.inventoryType===PRIMARY)return'Anticorps primaire';
    if(hit?.inventoryType===SEC)return'Anticorps secondaire';
    return String(hit?.inventoryType||'Produit').replaceAll('_',' ');
  }
  function openLocalReference(hit){
    if(!hit?.row)return;
    const row=hit.row;
    closeModal();
    if(hit.inventoryType===PRIMARY){state.selectedAntibody=Number(row.id);go('antibody-detail');return}
    const t=hit.inventoryType;if(!t)return toast('Type d’inventaire introuvable.');
    v112.inventorySelected=v112.inventorySelected||{};v112.inventoryView=v112.inventoryView||{};v112.inventoryDetailTab=v112.inventoryDetailTab||{};
    v112.inventorySelected[t]=Number(row.id);v112.inventoryView[t]='detail';v112.inventoryDetailTab[t]='overview';
    try{v112.renderInventory?.(t)}catch(err){console.warn('v11.7.5.4 open local inventory:',err);try{go(`inv:${t}`)}catch(_){}}
  }
  ...
  async function fetchReader(url){let last=null;for(const endpoint of ['https://r.jina.ai/','https://eu.r.jina.ai/']){try{return await fetchReaderOnce(endpoint,url)}catch(err){last=err;console.warn('v11.7.5.4 Reader:',endpoint,err)}}throw last||Error('Reader indisponible')}
  ...
  const PROVIDER_SEARCH={
    ...
    'cell signaling technology':'https://www.cellsignal.com/search?Ntt={catalog}',
    'santa cruz biotechnology':'https://www.scbt.com/search?keywords={catalog}',
    'biolegend':'https://www.biolegend.com/en-us/search-results?searchTerm={catalog}',
    'jackson immunoresearch':'https://www.jacksonimmuno.com/catalog/products?search={catalog}',
    'cytiva':'https://www.cytivalifesciences.com/en/us/search-results?query={catalog}',
    'merck sigma aldrich':'https://www.sigmaaldrich.com/FR/fr/search/{catalog}?focus=products&page=1&perpage=30&term={catalog}&type=product'
  };
  function supplierDescriptor(name){
    const key=supplierKey(name),known=URL_SUPPLIERS.find(s=>supplierKey(s.name)===key||s.key===key);
    return known||{key,name:String(name||'').trim()||'Fournisseur',test:()=>false};
  }
  function supplierSearchUrl(name,catalog){
    try{
      const cfg=tr('SupplierConfig').find(x=>[x.Name,x.CanonicalName,...String(x.Aliases||'').split('|')].some(a=>supplierSame(a,name)));
      if(cfg?.SearchTemplate)return String(cfg.SearchTemplate).replaceAll('{catalog}',encodeURIComponent(catalog)).replaceAll('{query}',encodeURIComponent(catalog));
    }catch(_){}
    const tpl=PROVIDER_SEARCH[supplierKey(name)];
    return tpl?tpl.replaceAll('{catalog}',encodeURIComponent(catalog)):'';
  }
  function readerLinks(text,baseUrl){
    const out=[],md=/\[([^\]]*)\]\(([^)\s]+)\)/gi,raw=/https?:\/\/[^\s<>"')\]]+/gi;let m;
    while((m=md.exec(String(text||'')))){const href=String(m[2]||'').replace(/&amp;/g,'&');if(/^data:|^javascript:/i.test(href))continue;try{out.push({label:cleanMarkdown(m[1]),url:new URL(href,baseUrl).href})}catch(_){}}
    while((m=raw.exec(String(text||'')))){try{out.push({label:'',url:new URL(m[0].replace(/[.,;:]+$/,'')).href})}catch(_){}}
    return [...new Map(out.map(x=>[x.url,x])).values()];
  }
  function sameSearchDomain(a,b){try{const ha=new URL(a).hostname.toLowerCase(),hb=new URL(b).hostname.toLowerCase();return ha===hb||ha.endsWith('.'+hb)||hb.endsWith('.'+ha)}catch(_){return false}}
  function productUrlScore(link,searchUrl,supplier,ref){
    let score=-999;try{
      const u=new URL(link.url),refc=compact(ref),urlc=compact(decodeURIComponent(u.pathname)),labelc=compact(link.label),sameKnown=supplier.test?.(u.hostname.toLowerCase()),sameSearch=sameSearchDomain(link.url,searchUrl);
      if(!sameKnown&&!sameSearch)return-999;score=0;
      if(urlc.includes(refc))score+=180;if(labelc.includes(refc))score+=90;
      if(/\/products?\/|\/antibody\/product\/|\/antibod(y|ies)\/|\/catalog\/products?\//i.test(u.pathname))score+=45;
      if(/\/search|search-results|keywords=|query=|searchterm=/i.test(link.url))score-=260;
      if(link.url.replace(/#.*$/,'')===searchUrl.replace(/#.*$/,''))score-=500;
      if(!urlc.includes(refc)&&!labelc.includes(refc))score-=160;
    }catch(_){return-999}return score;
  }
  function findExactProductUrl(text,searchUrl,supplier,ref){
    const ranked=readerLinks(text,searchUrl).map(x=>({...x,score:productUrlScore(x,searchUrl,supplier,ref)})).sort((a,b)=>b.score-a.score);
    return ranked.find(x=>x.score>=180)?.url||'';
  }

  function parseReaderJson(text){
    const raw=String(text||'').trim(),tries=[raw];
    const fenced=raw.match(/```(?:json)?\s*([\s\S]*?)```/i);if(fenced)tries.push(fenced[1].trim());
    const firstObj=raw.indexOf('{'),lastObj=raw.lastIndexOf('}');if(firstObj>=0&&lastObj>firstObj)tries.push(raw.slice(firstObj,lastObj+1));
    const firstArr=raw.indexOf('['),lastArr=raw.lastIndexOf(']');if(firstArr>=0&&lastArr>firstArr)tries.push(raw.slice(firstArr,lastArr+1));
    for(const s of tries){try{return JSON.parse(s)}catch(_){}}
    return null;
  }
  function registryExact(raw,ref){
    const wanted=compact(ref);
    if(compact(raw?.catalogNum)===wanted)return true;
    const alt=Array.isArray(raw?.catAlt)?raw.catAlt:String(raw?.catAlt||'').split(/[|,;]/);
    return alt.some(x=>compact(x)===wanted);
  }
  function registryToProduct(raw){
    const data={
      name:raw?.abName||'',supplier:raw?.vendorName||'',catalogNumber:raw?.catalogNum||'',
      target:raw?.abTarget||'',targetSpecies:Array.isArray(raw?.targetSpecies)?raw.targetSpecies.join(', '):String(raw?.targetSpecies||''),
      hostSpecies:Array.isArray(raw?.sourceOrganism)?raw.sourceOrganism.join(', '):String(raw?.sourceOrganism||''),
      className:Array.isArray(raw?.productIsotype)?raw.productIsotype.join(', '):String(raw?.productIsotype||raw?.clonality||''),
      fluorophore:Array.isArray(raw?.productConjugate)?raw.productConjugate.join(', '):String(raw?.productConjugate||''),
      applications:Array.isArray(raw?.applications)?raw.applications.join(', '):String(raw?.applications||''),
      website:raw?.vendorUrl||raw?.url||'',clonality:Array.isArray(raw?.clonality)?raw.clonality.join(', '):String(raw?.clonality||''),
      cloneId:Array.isArray(raw?.cloneId)?raw.cloneId.join(', '):String(raw?.cloneId||''),
      rrid:raw?.abId?(String(raw.abId).toUpperCase().startsWith('AB_')?String(raw.abId):`AB_${raw.abId}`):'',sourceMap:{}
    };
    for(const k of Object.keys(data)){if(k!=='sourceMap'&&usefulValue(data[k]))data.sourceMap[k]={kind:'registry',label:'Antibody Registry'}}
    return data;
  }
  async function registryReferenceSearch(ref,supplierName=''){
    const url=`https://www.antibodyregistry.org/api/fts-antibodies?q=${encodeURIComponent(ref)}&size=50&page=1`;
    try{
      const reader=await fetchReader(url),body=parseReaderJson(reader.text);
      const rawItems=Array.isArray(body)?body:Array.isArray(body?.items)?body.items:Array.isArray(body?.results)?body.results:[];
      let items=rawItems.filter(x=>registryExact(x,ref));
      if(supplierName){
        const exact=items.filter(x=>supplierSame(x?.vendorName,supplierName));
        if(exact.length)items=exact;
      }
      return{source:'Antibody Registry',reader,items:items.slice(0,8).map(registryToProduct),error:''};
    }catch(err){
      console.warn('v11.7.5.4 Antibody Registry via Reader:',err);
      return{source:'Antibody Registry',reader:null,items:[],error:String(err?.message||err)};
    }
  }
  async function supplierReferenceSearch(ref,supplierName){
    const searchUrl=supplierSearchUrl(supplierName,ref);
    if(!searchUrl)return{source:'Fiche fournisseur',reader:null,items:[],error:'Aucune recherche fournisseur configurée.',searchUrl:''};
    try{
      const supplier=supplierDescriptor(supplierName),searchReader=await fetchReader(searchUrl),productUrl=findExactProductUrl(searchReader.text,searchUrl,supplier,ref);
      if(!productUrl)return{source:'Fiche fournisseur',reader:searchReader,items:[],error:'Aucune fiche produit exacte détectée.',searchUrl};
      const productReader=await fetchReader(productUrl),data=extractFromSupplierUrl(productUrl,productReader.text,detectUrlSupplier(productUrl));
      data.catalogNumber=ref;data.supplier=data.supplier||supplierName;data.website=productUrl;data.sourceMap=data.sourceMap||{};
      data.sourceMap.catalogNumber={kind:'typed',label:'Référence recherchée'};
      return{source:'Fiche fournisseur',reader:productReader,items:[data],error:'',searchUrl};
    }catch(err){
      console.warn('v11.7.5.4 supplier reference search:',err);
      return{source:'Fiche fournisseur',reader:null,items:[],error:String(err?.message||err),searchUrl};
    }
  }
  function mergeInternetResults(results){
    const out=[];
    for(const r of results||[])for(const data of r.items||[]){
      const key=[compact(data.catalogNumber),supplierKey(data.supplier||''),compact(data.website||'')].join('|');
      const same=out.find(x=>{
        const cat=compact(x.data.catalogNumber)===compact(data.catalogNumber);
        const supplierOk=!x.data.supplier||!data.supplier||supplierSame(x.data.supplier,data.supplier);
        const urlOk=x.data.website&&data.website&&x.data.website===data.website;
        return cat&&(supplierOk||urlOk);
      });
      if(!same){out.push({data:{...data,sourceMap:{...(data.sourceMap||{})}},sources:[r.source],reader:r.reader});continue}
      for(const [k,v] of Object.entries(data)){if(k==='sourceMap')continue;if(!usefulValue(same.data[k])&&usefulValue(v))same.data[k]=v}
      same.data.sourceMap={...(data.sourceMap||{}),...(same.data.sourceMap||{})};
      if(!same.sources.includes(r.source))same.sources.push(r.source);
      same.reader=same.reader||r.reader;
    }
    return out;
  }
  ...
  function referenceCandidate(ref,supplier,url=''){
    const d={sourceMap:{},catalogNumber:String(ref||'').trim(),supplier:String(supplier||'').trim(),website:String(url||'').trim()};
    if(d.catalogNumber)d.sourceMap.catalogNumber={kind:'typed',label:'Référence saisie par le laboratoire'};
    if(d.supplier)d.sourceMap.supplier={kind:'typed',label:'Fournisseur saisi par le laboratoire'};
    if(d.website)d.sourceMap.website={kind:'url',label:'URL fournie par le laboratoire'};
    return d;
  }
  ...
  function showAddWizard(type,opts={}){
    ...
    modal(`<div class="v1174-add v1175-url-add"><p class="u127-eyebrow">AJOUT PRODUIT · ${e(profile.title.toUpperCase())}</p><h2>Ajouter depuis la fiche fournisseur</h2><p class="subtitle">Méthode recommandée : colle l’URL officielle du produit. Si elle ne peut pas être lue, BioDynaMit te demandera la référence catalogue et le fournisseur. Tout reste limité à ce parcours d’ajout.</p><div class="field"><label>URL officielle du produit *</label><input id="v1175ProductUrl" type="url" autocomplete="off" placeholder="https://www.abcam.com/... ou https://www.thermofisher.com/..."></div><div id="v1175UrlStatus" style="margin-top:12px"></div><div class="row" style="justify-content:flex-end;margin-top:18px"><button class="btn" id="v1175AddCancel">Annuler</button><button class="btn" id="v1175NoUrl">Je n’ai pas l’URL</button><button class="btn" id="v1175Manual">Saisie manuelle</button><button class="btn btn-primary" id="v1175Analyze">Analyser la fiche</button></div></div>`);
    document.querySelector('#v1175AddCancel').onclick=closeModal;
    document.querySelector('#v1175NoUrl').onclick=()=>showReferenceFallback(type,opts,{});
    document.querySelector('#v1175Manual').onclick=()=>showUrlPreview(type,{data:manualCandidate(type),opts,reader:null,manual:true,origin:'manual'});
    const run=()=>analyzeProductUrl(type,opts);document.querySelector('#v1175Analyze').onclick=run;document.querySelector('#v1175ProductUrl').onkeydown=ev=>{if(ev.key==='Enter'){ev.preventDefault();run()}};requestAnimationFrame(()=>document.querySelector('#v1175ProductUrl')?.focus());
  }
  function showReferenceFallback(type,opts={},ctx={}){
    const profile=productProfile(type),supplierGuess=ctx.supplier||'';
    modal(`<div class="v1174-add v1175-ref-add"><p class="u127-eyebrow">AJOUT PRODUIT · ${e(profile.title.toUpperCase())}</p><h2>Rechercher avec la référence catalogue</h2><p class="subtitle">La référence lance une recherche <b>dans BioDynaMit</b> et <b>sur internet</b>. Pour un anticorps, BioDynaMit interroge aussi l’Antibody Registry. Le fournisseur est facultatif pour la recherche locale et les anticorps, mais recommandé pour retrouver la fiche officielle.</p>${ctx.reason?`<div class="banner warn">${e(ctx.reason)}</div>`:''}<div class="form-grid"><div class="field"><label>Référence catalogue *</label><input id="v11752Ref" autocomplete="off" value="${e(ctx.ref||'')}" placeholder="Ex. ab150075, A-21244…"></div><div class="field"><label>Fournisseur <span class="u125-optional">recommandé</span></label><input id="v11752Supplier" autocomplete="off" list="v11752SupplierList" value="${e(supplierGuess)}" placeholder="Ex. Abcam, Thermo Fisher…"><datalist id="v11752SupplierList">${URL_SUPPLIERS.map(s=>`<option value="${e(s.name)}"></option>`).join('')}</datalist></div></div><div id="v11752RefStatus" style="margin-top:12px"></div><div class="row" style="justify-content:flex-end;margin-top:18px"><button class="btn" id="v11752RefBack">Retour</button><button class="btn" id="v11752RefManual">Continuer manuellement</button><button class="btn btn-primary" id="v11752RefSearch">Rechercher</button></div></div>`);
    document.querySelector('#v11752RefBack').onclick=()=>showAddWizard(type,opts);
    const values=()=>({ref:String(document.querySelector('#v11752Ref')?.value||'').trim(),supplier:String(document.querySelector('#v11752Supplier')?.value||'').trim()});
    const run=()=>analyzeProductReference(type,opts,{url:ctx.url||''});
    document.querySelector('#v11752RefSearch').onclick=run;
    document.querySelector('#v11752RefManual').onclick=()=>{const {ref,supplier}=values();if(!ref)return toast('La référence catalogue est obligatoire.');showUrlPreview(type,{data:referenceCandidate(ref,supplier,ctx.url||''),opts,reader:null,manual:false,origin:'reference',lookupNote:'Aucune recherche automatique utilisée. Vérifie et complète les champs avant création.'})};
    document.querySelector('#v11752Ref').onkeydown=ev=>{if(ev.key==='Enter'){ev.preventDefault();run()}};
    document.querySelector('#v11752Supplier').onkeydown=ev=>{if(ev.key==='Enter'){ev.preventDefault();run()}};
    requestAnimationFrame(()=>document.querySelector('#v11752Ref')?.focus());
  }
  function showReferenceSearchResults(type,opts,{ref,supplierName,local,internet,errors,ctx={}}){
    const profile=productProfile(type),sameType=local.filter(h=>h.inventoryType===type&&activeRaw(h.row));
    const localHtml=local.length?`<div class="v11754-search-section"><h3>Dans BioDynaMit <span class="pill neutral">${local.length}</span></h3><div class="v11754-search-results">${local.map((h,i)=>`<div class="v11754-result"><div><b>${e(h.row.Name||h.row.FullName||h.row.Code||'Produit')}</b><small>${e(localTypeLabel(h))} · ${e(h.row.Supplier||'—')} · ${e(h.row.CatalogNumber||ref)}${activeRaw(h.row)?'':' · ARCHIVÉ'}${supplierName&&!h.supplierMatch?' · fournisseur différent':''}</small></div><button class="btn" data-v11754-local="${i}">${activeRaw(h.row)?'Ouvrir':'Voir la fiche archivée'}</button></div>`).join('')}</div></div>`:`<div class="v11754-search-section"><h3>Dans BioDynaMit</h3><div class="empty">Aucune référence exacte trouvée dans le stock actuel.</div></div>`;
    const internetHtml=internet.length?`<div class="v11754-search-section"><h3>Sur internet <span class="pill ok">${internet.length}</span></h3><div class="v11754-search-results">${internet.map((x,i)=>`<div class="v11754-result"><div><b>${e(x.data.name||x.data.catalogNumber||ref)}</b><small>${e(x.data.supplier||supplierName||'Fournisseur non identifié')} · ${e(x.data.catalogNumber||ref)} · ${e(x.sources.join(' + '))}</small>${x.data.website?`<a class="v11754-mini-link" href="${e(x.data.website)}" target="_blank" rel="noopener noreferrer">Fiche source ↗</a>`:''}</div><button class="btn btn-primary" data-v11754-online="${i}">${sameType.length?'Voir les données':'Utiliser ces données'}</button></div>`).join('')}</div></div>`:`<div class="v11754-search-section"><h3>Sur internet</h3><div class="empty">Aucune fiche exacte n’a été retrouvée automatiquement.${!supplierName&&type!==PRIMARY&&type!==SEC?' Indique un fournisseur pour rechercher un produit générique.':''}</div></div>`;
    const errText=(errors||[]).filter(Boolean).join(' · ');
    modal(`<div class="v1174-add v11754-reference-results"><p class="u127-eyebrow">AJOUT PRODUIT · RECHERCHE PAR RÉFÉRENCE</p><div class="row space-between" style="align-items:flex-start"><div><h2>Résultats pour ${e(ref)}</h2><p class="subtitle">BioDynaMit et sources internet sont interrogés séparément.</p></div><span class="pill ${local.length||internet.length?'ok':'warn'}">${local.length+internet.length} résultat${local.length+internet.length>1?'s':''}</span></div>${sameType.length?`<div class="banner warn"><b>Cette référence existe déjà dans l’inventaire ${e(profile.title)}.</b> Les résultats internet restent visibles pour comparaison, mais une nouvelle création sera bloquée pour éviter un doublon.</div>`:''}${errText?`<details class="v11754-errors"><summary>Détails des recherches non concluantes</summary><p class="v1174-source warn">${e(errText)}</p></details>`:''}${localHtml}${internetHtml}<div class="row" style="justify-content:flex-end;margin-top:18px"><button class="btn" id="v11754Again">Nouvelle recherche</button><button class="btn" id="v11754Manual">Continuer avec la référence</button></div></div>`);
    document.querySelectorAll('[data-v11754-local]').forEach(b=>b.onclick=()=>openLocalReference(local[Number(b.dataset.v11754Local)]));
    document.querySelectorAll('[data-v11754-online]').forEach(b=>b.onclick=()=>{
      const x=internet[Number(b.dataset.v11754Online)];if(!x)return;
      showUrlPreview(type,{data:x.data,opts,reader:x.reader||{endpoint:'https://r.jina.ai/'},manual:false,origin:'reference',lookupNote:`Résultat internet : ${x.sources.join(' + ')}. Vérifie les valeurs avant création.`});
    });
    document.querySelector('#v11754Again').onclick=()=>showReferenceFallback(type,opts,{ref,supplier:supplierName,url:ctx.url||''});
    document.querySelector('#v11754Manual').onclick=()=>showUrlPreview(type,{data:referenceCandidate(ref,supplierName,ctx.url||''),opts,reader:null,manual:false,origin:'reference',lookupNote:'Référence conservée sans donnée internet supplémentaire. Complète les champs manquants avant création.'});
  }
  async function analyzeProductReference(type,opts={},ctx={}){
    const ref=String(document.querySelector('#v11752Ref')?.value||'').trim(),supplierName=String(document.querySelector('#v11752Supplier')?.value||'').trim();
    if(!ref)return toast('La référence catalogue est obligatoire.');
    const status=document.querySelector('#v11752RefStatus'),btn=document.querySelector('#v11752RefSearch');
    if(btn){btn.disabled=true;btn.textContent='Recherche…'}
    if(status)status.innerHTML='<div class="banner neutral"><b>Recherche en cours…</b><br>Stock BioDynaMit + sources internet disponibles.</div>';
    try{
      await refreshAll();
      const local=localReferenceMatches(ref,supplierName);
      const internetJobs=[];
      if(supplierName)internetJobs.push(supplierReferenceSearch(ref,supplierName));
      if(type===PRIMARY||type===SEC)internetJobs.push(registryReferenceSearch(ref,supplierName));
      const results=internetJobs.length?await Promise.all(internetJobs):[];
      const internet=mergeInternetResults(results),errors=results.map(r=>r.error?`${r.source} : ${r.error}`:'').filter(Boolean);
      showReferenceSearchResults(type,opts,{ref,supplierName,local,internet,errors,ctx});
    }catch(err){
      console.error('v11.7.5.4 reference search:',err);
      showReferenceSearchResults(type,opts,{ref,supplierName,local:localReferenceMatches(ref,supplierName),internet:[],errors:[String(err?.message||err)],ctx});
    }finally{if(btn?.isConnected){btn.disabled=false;btn.textContent='Rechercher'}}
  }
  async function analyzeProductUrl(type,opts={}){
    const input=document.querySelector('#v1175ProductUrl'),typed=String(input?.value||'').trim();if(!typed)return toast('Colle l’URL officielle du produit.');const url=normalizeProductUrl(typed);if(input&&url!==typed)input.value=url;let parsed,supplier;try{parsed=new URL(url);if(!/^https?:$/.test(parsed.protocol))throw Error('URL non HTTP(S)');supplier=detectUrlSupplier(url)}catch(err){return toast('URL fournisseur invalide.')}const btn=document.querySelector('#v1175Analyze'),status=document.querySelector('#v1175UrlStatus');btn.disabled=true;btn.textContent='Analyse…';status.innerHTML=`<div class="banner neutral">Lecture de la fiche ${e(supplier.name)}…</div>`;try{const reader=await fetchReader(url),data=extractFromSupplierUrl(url,reader.text,supplier);if(checkExistingAfterAnalysis(type,data,opts))return;showUrlPreview(type,{data,opts,reader,manual:false,origin:'url'})}catch(err){console.error('v11.7.5.4 product URL:',err);const reason=err?.name==='AbortError'?'Délai de lecture dépassé.':String(err?.message||err);status.innerHTML=`<div class="banner warn"><b>La fiche n’a pas pu être lue automatiquement.</b><br>${e(reason)}<div class="row" style="margin-top:10px"><button class="btn btn-primary" id="v11752FallbackRef">Utiliser la référence catalogue</button><button class="btn" id="v1175FallbackManual">Saisie manuelle</button><a class="btn" href="${e(url)}" target="_blank" rel="noopener noreferrer">Ouvrir la fiche fournisseur ↗</a></div></div>`;document.querySelector('#v11752FallbackRef').onclick=()=>showReferenceFallback(type,opts,{url,supplier:supplier.name,reason:'La lecture de l’URL a échoué. Utilise maintenant la référence catalogue du produit.'});document.querySelector('#v1175FallbackManual').onclick=()=>showUrlPreview(type,{data:manualCandidate(type,url),opts,reader:null,manual:true,origin:'manual'});btn.disabled=false;btn.textContent='Analyser la fiche'}
  }
  function showUrlPreview(type,{data,opts,reader,manual,origin='url',lookupNote='',searchUrl=''}){
    const profile=productProfile(type),found=profile.fields.filter(([k])=>usefulValue(data?.[k])).length,mode=manual?'manual':origin==='reference'?'reference':'url';
    const subtitle=mode==='manual'?'Saisie manuelle':mode==='reference'?'Préremplissage depuis la référence catalogue':'Préremplissage depuis l’URL fournisseur';
    const badge=mode==='manual'?'Manuel':mode==='reference'?'Référence':'URL fournisseur';
    const prov=mode==='manual'?'saisie manuelle':mode==='reference'?(reader?'référence + fournisseur → fiche fournisseur lue via Reader':'référence catalogue + fournisseur saisis par le laboratoire'):`fiche fournisseur lue via Reader${reader?.endpoint?` (${e(reader.endpoint.replace('https://','').replace('/',''))})`:''}`;
    modal(`<div class="v1174-add v1175-product-preview"><p class="u127-eyebrow">AJOUT PRODUIT · ${e(profile.title.toUpperCase())}</p><div class="row space-between" style="align-items:flex-start;gap:12px"><div><h2>Vérifier avant création</h2><p class="subtitle">${e(subtitle)} · ${found}/${profile.fields.length} champs du profil renseignés</p></div><span class="pill ${mode==='manual'?'neutral':mode==='reference'?'warn':'ok'}">${e(badge)}</span></div>${lookupNote?`<div class="banner ${reader?'ok':'warn'}">${e(lookupNote)}${searchUrl?` <a href="${e(searchUrl)}" target="_blank" rel="noopener noreferrer">Ouvrir la recherche fournisseur ↗</a>`:''}</div>`:''}${data?.website?`<div class="row" style="margin:10px 0"><a class="btn" href="${e(data.website)}" target="_blank" rel="noopener noreferrer">Ouvrir la fiche fournisseur ↗</a></div>`:''}<div class="form-grid">${profile.fields.map(([k,l,t])=>profileFieldHtml(k,l,t,data)).join('')}</div>${extraDetectedHtml(type,data)}<div class="field" style="margin-top:12px"><label>Commentaires</label><textarea id="v1175Comments" rows="3"></textarea><small class="v1174-source neutral">Validation humaine obligatoire : aucune valeur n’est enregistrée avant ton clic sur « Créer la référence ».</small></div><div class="v1174-provenance"><b>Provenance :</b> ${prov}.</div><div class="row" style="justify-content:flex-end;margin-top:18px"><button class="btn" id="v1175PreviewBack">Retour</button><button class="btn btn-primary" id="v1175Create">Créer la référence</button></div></div>`);
    document.querySelector('#v1175PreviewBack').onclick=()=>showAddWizard(type,opts);document.querySelector('#v1175Create').onclick=()=>createReferenceFromUrl(type,{data,opts,manual,origin});
  }
  ...
  function installStyle(){if(document.getElementById('v1174Style'))return;const s=document.createElement('style');s.id='v1174Style';s.textContent='.v1174-source{display:block;margin-top:5px;font-size:10px;color:#64788a}.v1174-source.ok{color:#25704f}.v1174-source.warn{color:#9a6820}.v1174-provenance{margin-top:14px;padding:10px 12px;border:1px solid var(--line);border-radius:10px;background:#f7fafc;font-size:12px;color:#536a7c}.v1174-stock-tools .pill{min-width:120px;text-align:center}.v1174-add .banner,.v1174-assign .banner{margin-top:10px}.v11754-search-section{margin-top:16px}.v11754-search-section h3{display:flex;align-items:center;gap:8px;margin:0 0 8px}.v11754-search-results{display:grid;gap:8px}.v11754-result{display:flex;justify-content:space-between;align-items:center;gap:12px;border:1px solid var(--line);border-radius:10px;padding:10px 12px;background:#fff}.v11754-result b,.v11754-result small{display:block}.v11754-result small{margin-top:3px;color:#64788a}.v11754-mini-link{display:inline-block;margin-top:4px;font-size:11px}.v11754-errors{margin-top:10px}';document.head.appendChild(s)}
  function install(){
  ...
  W.physicalStockState=physicalStockState;W.showAssignPosition=showAssignPosition;W.showAddWizard=showAddWizard;W.showProductUrlWizard=showAddWizard;W.productProfiles=PRODUCT_PROFILES;W.extractFromSupplierUrl=extractFromSupplierUrl;W.showArchived=showArchived;W._test={pendingSecondaryUnit,primaryDependencies,secondaryDependencies,catalogFromReader,titleFromReader,findReaderField,detectUrlSupplier,normalizeProductUrl,supplierSame,sameTypeRefs,localReferenceMatches,supplierSearchUrl,findExactProductUrl,referenceCandidate,parseReaderJson,registryExact,registryToProduct,mergeInternetResults};
  install();
})();
