...
(function(){
  ...
  const OPEN_ORDER_STATUSES=new Set(['a commander','commande','recu']);
  function isOpenOrder(o){return OPEN_ORDER_STATUSES.has(norm(o?.Status||''));}
  function orderKey(type,code){return `${String(type||'')}|${String(code||'')}`;}
  function pendingOrders(){return tableRows('LabOrders').filter(isOpenOrder).sort((a,b)=>Number(b.DateRequested||0)-Number(a.DateRequested||0));}
  function openOrderFor(item){return item?pendingOrders().find(o=>orderKey(o.InventoryType,o.ItemCode)===orderKey(item.inventoryType,item.code))||null:null;}
  function orderButton(item,label='Commander'){
    const open=openOrderFor(item);
    return open
      ? `<button class="btn btn-sm" data-v115-open-order="${Number(open.id)}">✓ Commande en cours</button>`
      : `<button class="btn btn-sm" data-v115-order="${eh(item.inventoryType)}|${eh(item.code)}">${eh(label)}</button>`;
  }
  function lowStockItems(){
    const ordered=new Set(pendingOrders().map(o=>orderKey(o.InventoryType,o.ItemCode)));
    return allItems().filter(i=>!ordered.has(orderKey(i.inventoryType,i.code))&&(i.stock===0 || (i.stock===1&&i.lowCount>0) || (i.stock>0&&i.lowCount>=i.stock))).sort((a,b)=>a.stock-b.stock||a.name.localeCompare(b.name));
  }
  ...
  function overviewHtml(){
    ...
    <div class="v115-grid-2" style="margin-top:16px">
      <div class="card card-pad"><div class="v115-card-title"><h3>Stock à surveiller</h3><span class="pill warn">${low.length}</span></div>
        ${low.length?`<div class="v115-list">${low.slice(0,8).map(i=>`<div class="v115-list-row"><div class="meta"><b>${eh(i.name)}</b><small>${eh(i.supplier)} · ${eh(i.catalogNumber||i.code)}</small></div><div>${stockPill(i)} ${orderButton(i,'Commander')}</div></div>`).join('')}</div>`:'<div class="empty">Aucun stock critique détecté.</div>'}
      </div>
  ...
  function ordersHtml(){
    ...
    return `<div class="v115-grid-2"><div class="card card-pad"><h2>Suggestions de réapprovisionnement</h2><p class="subtitle">Calculées à partir du stock actuel. Une référence avec commande ouverte change de catégorie et n’apparaît plus ici.</p>${low.length?`<div class="v115-list">${low.slice(0,20).map(i=>`<div class="v115-list-row"><div class="meta"><b>${eh(i.name)}</b><small>${eh(i.supplier)} · ${eh(i.catalogNumber||i.code)}</small></div><div>${stockPill(i)} ${orderButton(i,'Ajouter')}</div></div>`).join('')}</div>`:'<div class="empty">Aucune suggestion.</div>'}</div>
      <div class="card card-pad"><h2>À traiter</h2><p class="subtitle">Cycle : À commander → Commandé → Reçu → Rangé.</p>${list.length?`<div class="v115-list">${list.map(o=>`<div class="v115-list-row" data-v115-order-row="${Number(o.id)}"><div class="meta"><b>${eh(o.ItemName||o.ItemCode)}</b><small>${eh(o.Supplier)} · ${eh(o.CatalogNumber)} · ${fmtDate(o.DateRequested)}</small><span>${eh(o.Reason||'')}</span></div><div style="text-align:right"><span class="pill ${orderStatusClass(o.Status)}">${eh(o.Status||'À commander')}</span><div style="margin-top:6px">${orderActionButton(o)}</div></div></div>`).join('')}</div>`:'<div class="empty">Aucune commande enregistrée.</div>'}</div></div>`;
  }
  ...
  function openExistingOrder(id){team.tab='orders';if(state.route!=='team-tools')go('team-tools');else renderTeamTools();requestAnimationFrame(()=>document.querySelector(`[data-v115-order-row="${Number(id)}"]`)?.scrollIntoView({behavior:'smooth',block:'center'}));}
  function bindOrderButtons(){document.querySelectorAll('[data-v115-order]').forEach(b=>b.onclick=()=>{const [type,...rest]=b.dataset.v115Order.split('|');showOrderModal(itemByCode(rest.join('|'),type));});document.querySelectorAll('[data-v115-open-order]').forEach(b=>b.onclick=()=>openExistingOrder(b.dataset.v115OpenOrder));}
  ...
  function showOrderModal(item){if(!item)return toast('Référence introuvable.');const existing=openOrderFor(item);if(existing){toast('Une commande ouverte existe déjà pour cette référence.');openExistingOrder(existing.id);return;}modal(`<h2>Ajouter à la liste de commande</h2><p><b>${eh(item.name)}</b><br>${eh(item.supplier)} · ${eh(item.catalogNumber||item.code)}</p><div class="form-grid"><div class="field"><label>Quantité</label><input id="v115OrdQty" value="1"></div><div class="field"><label>Demandé par</label><input id="v115OrdBy" placeholder="Nom / initiales"></div></div><div class="field"><label>Motif</label><input id="v115OrdReason" value="${item.stock===0?'Stock épuisé':'Stock faible'}"></div><div class="field"><label>Notes</label><textarea id="v115OrdNotes" rows="3"></textarea></div><div class="row" style="justify-content:flex-end"><button class="btn" id="v115OrdCancel">Annuler</button><button class="btn btn-primary" id="v115OrdSave">Ajouter</button></div>`);document.querySelector('#v115OrdCancel').onclick=closeModal;document.querySelector('#v115OrdSave').onclick=async()=>{const fresh=openOrderFor(item);if(fresh){closeModal();toast('Une commande ouverte existe déjà pour cette référence.');openExistingOrder(fresh.id);return;}const rec={DateRequested:nowSec(),InventoryType:item.inventoryType,ItemCode:item.code,ItemName:item.name,Supplier:item.supplier,CatalogNumber:item.catalogNumber,Reason:document.querySelector('#v115OrdReason').value.trim(),Quantity:document.querySelector('#v115OrdQty').value.trim(),Status:'À commander',RequestedBy:document.querySelector('#v115OrdBy').value.trim(),DateOrdered:null,DateReceived:null,Notes:document.querySelector('#v115OrdNotes').value.trim()};try{await write([['AddRecord','LabOrders',null,rec]]);closeModal();toast('Ajouté à la liste de commande.');if(state.route==='team-tools')renderTeamTools();else render();}catch(err){toast(`Erreur : ${err.message||err}`);}};}
  ...
  team.refresh=refreshTeamData;team.initSchema=initTeamSchema;team.render=renderTeamTools;team.allItems=allItems;team.allUnits=allUnits;team.openOrderFor=openOrderFor;team.showOrderModal=showOrderModal;team.openExistingOrder=openExistingOrder;team.qrMatrix=qrMatrix;team.qrSvg=qrSvg;team.resolveScan=resolveScan;team._orderTest={isOpenOrder,orderKey,pendingOrders,lowStockItems};
  bootstrap();
})();
