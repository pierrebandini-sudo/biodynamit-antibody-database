/* BioDynaMit v11.7.5.2 — stockage unifié, ajout par URL fournisseur et gestion des références.
...
(function(){
  ...
  /* --------------------- Product creation by supplier URL --------------------
     v11.7.5.2 scope rule: this Reader/enrichment UI exists ONLY inside an explicit
  ...
  function sameTypeRefs(type,ref,supplier=''){
    ...
    const rows0=type===PRIMARY?tr('Antibodies'):tr('InventoryItems').filter(i=>i.InventoryType===SEC);
  ...
  async function fetchReader(url){let last=null;for(const endpoint of ['https://r.jina.ai/','https://eu.r.jina.ai/']){try{return await fetchReaderOnce(endpoint,url)}catch(err){last=err;console.warn('v11.7.5.2 Reader:',endpoint,err)}}throw last||Error('Reader indisponible')}
  ...
  function showReferenceFallback(type,opts={},ctx={}){
    ...
    modal(`<div class="v1174-add v1175-ref-add"><p class="u127-eyebrow">AJOUT PRODUIT · ${e(profile.title.toUpperCase())}</p><h2>Retrouver le produit par sa référence</h2><p class="subtitle">Indique la référence catalogue et le fournisseur. BioDynaMit vérifie d’abord les doublons, puis essaie de retrouver la fiche officielle. S’il n’y arrive pas, ces deux informations restent préremplies pour la saisie manuelle.</p>${ctx.reason?`<div class="banner warn">${e(ctx.reason)}</div>`:''}<div class="form-grid"><div class="field"><label>Référence catalogue *</label><input id="v11752Ref" autocomplete="off" value="${e(ctx.ref||'')}" placeholder="Ex. ab150075, A-21244…"></div><div class="field"><label>Fournisseur *</label><input id="v11752Supplier" autocomplete="off" list="v11752SupplierList" value="${e(supplierGuess)}" placeholder="Ex. Abcam, Thermo Fisher…"><datalist id="v11752SupplierList">${URL_SUPPLIERS.map(s=>`<option value="${e(s.name)}"></option>`).join('')}</datalist></div></div><div id="v11752RefStatus" style="margin-top:12px"></div><div class="row" style="justify-content:flex-end;margin-top:18px"><button class="btn" id="v11752RefBack">Retour</button><button class="btn" id="v11752RefManual">Continuer sans recherche</button><button class="btn btn-primary" id="v11752RefSearch">Rechercher à partir de la référence</button></div></div>`);
    document.querySelector('#v11752RefBack').onclick=()=>showAddWizard(type,opts);
    document.querySelector('#v11752RefManual').onclick=()=>{const ref=String(document.querySelector('#v11752Ref')?.value||'').trim(),supplier=String(document.querySelector('#v11752Supplier')?.value||'').trim();if(!ref||!supplier)return toast('La référence catalogue et le fournisseur sont obligatoires.');const data=referenceCandidate(ref,supplier,ctx.url||'');if(checkExistingAfterAnalysis(type,data,opts))return;showUrlPreview(type,{data,opts,reader:null,manual:false,origin:'reference',lookupNote:'Aucune recherche automatique effectuée : complète les champs manquants avant création.'})};
    const run=()=>analyzeProductReference(type,opts,{url:ctx.url||''});document.querySelector('#v11752RefSearch').onclick=run;document.querySelector('#v11752Ref').onkeydown=ev=>{if(ev.key==='Enter'){ev.preventDefault();run()}};document.querySelector('#v11752Supplier').onkeydown=ev=>{if(ev.key==='Enter'){ev.preventDefault();run()}};requestAnimationFrame(()=>document.querySelector('#v11752Ref')?.focus());
  }
  async function analyzeProductReference(type,opts={},ctx={}){
    ...
    if(!ref||!supplierName)return toast('La référence catalogue et le fournisseur sont obligatoires.');
    const base=referenceCandidate(ref,supplierName,ctx.url||'');if(checkExistingAfterAnalysis(type,base,opts))return;
    const status=document.querySelector('#v11752RefStatus'),btn=document.querySelector('#v11752RefSearch'),supplier=supplierDescriptor(supplierName),searchUrl=supplierSearchUrl(supplierName,ref);
    btn.disabled=true;btn.textContent='Recherche…';status.innerHTML='<div class="banner neutral">Recherche de la fiche fournisseur…</div>';
    try{
      if(!searchUrl){status.innerHTML='<div class="banner warn">Aucun moteur fournisseur n’est configuré pour ce fournisseur. La référence et le fournisseur ont été conservés.</div>';showUrlPreview(type,{data:base,opts,reader:null,manual:false,origin:'reference',lookupNote:'Fiche officielle non retrouvée automatiquement. Complète les champs manquants avant création.'});return}
      const searchReader=await fetchReader(searchUrl),productUrl=findExactProductUrl(searchReader.text,searchUrl,supplier,ref);
      if(!productUrl){status.innerHTML=`<div class="banner warn">La recherche fournisseur a été lue, mais aucune fiche produit exacte n’a été identifiée pour <b>${e(ref)}</b>.</div>`;showUrlPreview(type,{data:base,opts,reader:null,manual:false,origin:'reference',lookupNote:'La fiche exacte n’a pas été retrouvée automatiquement. La référence et le fournisseur sont préremplis.',searchUrl});return}
